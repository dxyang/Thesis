...
def create_autoencoder(maskVecXoneYzero, bottleneck, learning_rate=1e-3):
  class Net:
    ...
    batch_mean3_b, batch_var3_b = tf.nn.moments(h_d_deconv3_b, axes=[0, 1, 2])
    scale3_b = tf.Variable(tf.ones([128]))
    beta3_b = tf.Variable(tf.zeros([128]))
    bn3_b = tf.nn.batch_normalization(h_d_deconv3_b, batch_mean3_b, batch_var3_b, beta3_b, scale3_b, epsilon)
    h_d_conv3_b = tf.nn.relu(bn3_b + b_d_conv3_b)
    ...
    batch_mean3, batch_var3 = tf.nn.moments(h_d_deconv3, axes=[0, 1, 2])
    scale3 = tf.Variable(tf.ones([64]))
    beta3 = tf.Variable(tf.zeros([64]))
    bn3 = tf.nn.batch_normalization(h_d_deconv3, batch_mean3, batch_var3, beta3, scale3, epsilon)
    h_d_conv3 = tf.nn.relu(bn3 + b_d_conv3)
    ...
    batch_mean2_b, batch_var2_b = tf.nn.moments(h_d_deconv2_b, axes=[0, 1, 2])
    scale2_b = tf.Variable(tf.ones([64]))
    beta2_b = tf.Variable(tf.zeros([64]))
    bn2_b = tf.nn.batch_normalization(h_d_deconv2_b, batch_mean2_b, batch_var2_b, beta2_b, scale2_b, epsilon)
    h_d_conv2_b = tf.nn.relu(bn2_b + b_d_conv2_b)
    ...
    bn2 = tf.nn.batch_normalization(h_d_deconv2, batch_mean2, batch_var2, beta2, scale2, epsilon)
    ...
    batch_mean1_b, batch_var1_b = tf.nn.moments(h_d_deconv1_b, axes=[0, 1, 2])
    scale1_b = tf.Variable(tf.ones([32]))
    beta1_b = tf.Variable(tf.zeros([32]))
    bn1_b = tf.nn.batch_normalization(h_d_deconv1_b, batch_mean1_b, batch_var1_b, beta1_b, scale1_b, epsilon)
    h_d_conv1_b = tf.nn.relu(bn1_b + b_d_conv1_b)
...
def create_autoencoder_adversarial(bottleneck, learning_rate=1e-3):
  class Net:
    ...
    g_vars = [W_e_conv1, b_e_conv1, W_e_conv1_b, b_e_conv1_b, 
                ...
                scale3_b, beta3_b, scale3, beta3,
                W_d_conv2_b, b_d_conv2_b, W_d_conv2, b_d_conv2,
                scale2_b, beta2_b, scale2, beta2,
    ...
    total_cost = 0.99*cost + 0.01*g_loss
